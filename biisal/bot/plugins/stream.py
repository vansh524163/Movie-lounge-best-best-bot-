...
async def private_receive_handler(c: Client, m: Message):
    ...
    try:
        ...
        file_link = f"https://telegram.me/{Var.SECOND_BOTUSERNAME}?start=file_{log_msg.id}/{quote_plus(get_name(log_msg))}?hash={get_hash(log_msg)}"
        share_link = f"https://ddlink57.blogspot.com/{str(log_msg.id)}"
        ...
        await m.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    ...
                    [
                        InlineKeyboardButton('⚡ Share Link ⚡', url=file_link)  # Corrected Share Link Button
                    ]
                ]
            )
        )
